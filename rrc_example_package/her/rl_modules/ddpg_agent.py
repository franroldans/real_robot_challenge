...
class ddpg_agent:
    def __init__(self, args, env, env_params):
        ...
        if MPI.COMM_WORLD.Get_rank() == 0:
            ...
            self.model_path = os.path.join(self.args.save_dir, 'difficulty1_2')
    ...
    def learn(self):
        ...
        for epoch in range(self.args.n_epochs):
            ...
            if MPI.COMM_WORLD.Get_rank() == 0:
                print('[{}] epoch is: {}, eval success rate is: {:.3f} , actor_loss: {:.2f}'.format(datetime.now(), epoch, success_rate, np.mean(actor_loss)))
                torch.save([self.o_norm.mean, self.o_norm.std, self.g_norm.mean, self.g_norm.std, self.actor_network.state_dict(), self.critic_network.state_dict()], \
                            self.model_path + '/model{}.pt'.format(epoch))
